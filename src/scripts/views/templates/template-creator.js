  ...
  <article class="restaurant__item" tabindex="0">
    <div class="restaurant__item__img">
      <img
        ...
        src="${CONFIG.SMALL_IMG_URL}${restaurant.pictureId}"
        loading="lazy"
      />
    </div>
    <div class="restaurant__item__detail" data-id=${restaurant.id}>
      <a href='/#/detail/${restaurant.id}' tabindex="0" class="restaurant__item__title">${restaurant.name}</a>
      <p class="restaurant__item__rating" tabindex="0">
        Rating: ${ratting(restaurant.rating)} <span>${restaurant.rating}</span>
      </p>
      <p tabindex="0" class="restaurant__item__desc">
        ${restaurant.description}
      </p>
    </div>
    <p tabindex="0" class="restaurant__item__city">${restaurant.city}</p>
  </article>
`;
...
const createDetailRestaurant = (restaurant) => `
  <img
    ...
    src="${CONFIG.MEDIUM_IMG_URL}${restaurant.pictureId}"
    alt="${restaurant.name}"
  />
  <section class="restaurant__detail__section" tabindex="0" id="#main">
    ...
    <div class="restaurant__detail__info" tabindex="0">
      <div class="ratting">⭐ ${restaurant.rating}</div>
    ...
    <div class="restaurant__category__container">
      Categories:
      <p class="restaurant__detail__category">
        ${restaurant.categories.map((element) => `${element.name}`).join('')}
      </p>
    </div>
    <section class="restaurant__detail__menu">
      ...
      <div class="food">
        <h3>Food</h3>
        <ul>
          ${restaurant.menus.foods.map((element) => `<li>${element.name}</li>`).join('')}
        </ul>
      </div>
      <div class="drink">
        <h3>Drink</h3>
        <ul>
          ${restaurant.menus.drinks.map((element) => `<li>${element.name}</li>`).join('')}
        </ul>
      </div>
    </section>
    <hr />
